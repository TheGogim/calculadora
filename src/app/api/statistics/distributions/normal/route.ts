...
import { NormalDistribution, NormalParams } from '@/lib/statistics/distributions/normal';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Error en API de Normal:', error);
    return NextResponse.json(
...
async function handleProbability(searchParams: URLSearchParams) {
  const mu = parseFloat(searchParams.get('mu') || '0');
  const sigma = parseFloat(searchParams.get('sigma') || '1');
  const x = parseFloat(searchParams.get('x') || '0');
  const type = searchParams.get('type') || 'le';

  // Validar parámetros
  const errors = NormalDistribution.validateParams({ mu, sigma });
  if (errors.length > 0) {
    return NextResponse.json(
...
async function handleTable(searchParams: URLSearchParams) {
  const mu = parseFloat(searchParams.get('mu') || '0');
  const sigma = parseFloat(searchParams.get('sigma') || '1');
  const start = parseFloat(searchParams.get('start') || '-3');
  const end = parseFloat(searchParams.get('end') || '3');
...
async function handleExamples() {
  const examples = NormalDistribution.getExamples();
